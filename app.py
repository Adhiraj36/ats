...
# Load sentence-transformers model (small & fast)
@st.cache_resource
...
@st.cache_data
def get_embedding(text: str, model) -> Optional[np.ndarray]:
    if not text.strip():
        return None
    try:
        emb = model.encode(text.replace("\n", " "), convert_to_numpy=True)
        return emb
    except Exception as e:
...
def evaluate_resume(resume_text, jd_text, embed_model):
...
def main():
    ...
    custom_css = """
    <style>
    .block-container { max-width: 1000px; padding: 2rem 2rem 2rem 2rem; }
    .stButton>button {
        background-color: #4B8BBE; color: white; font-weight: bold; border-radius: 8px;
        padding: 10px 20px; border: none; transition: background-color 0.3s ease;
    }
    .stButton>button:hover { background-color: #306998; color: white; }
    header .css-1v3fvcr { background-color: #14213d !important; color: white !important; }
        ...
        if st.button("🚀 Evaluate Resumes"):
            ...
            for i, uf in enumerate(uploaded_resumes):
                ...
                if not resume_text:
                    ...
                    continue
                res_eval = evaluate_resume(resume_text, job_desc_text, embed_model)
